...
모든 모듈은 strict TypeScript와 async/await를 사용하며 브라우저 API나 상호작용 훅이 필요한 경우에만 `'use client'`를 선언한다. 컴포넌트는 PascalCase, 훅은 camelCase, 유틸은 케밥 표기, 파일 길이는 200줄 이하로 유지하고 복잡한 로직은 `src/lib/`로 이동한다. Tailwind 커스텀 프로퍼티(`text-[var(--color-foreground)]`)를 활용해 모바일 퍼스트·노션 스타일(흰 배경, 얇은 회색 보더, 최소한의 포인트 컬러) 테마를 일관되게 유지한다.
...
## 기록 & 개발 메모
- OTP UI는 4자리 전용 커스텀 패드로 재작성되었다.
- UI 테마는 노션 스타일(흰 배경, `#37352F` 계열 텍스트, 얇은 회색 보더, 소량의 블루 포인트, 작은 라운드 `--radius: 0.375rem`)로 전면 교체되었다. 문구도 중립적으로 통일했다 — 앱 이름은 "Bang Storage", 개인화된 문구/색상은 사용하지 않는다. 컬러 토큰은 `src/app/globals.css`의 `:root`/`.dark`에서 관리한다.
- 로그인 이후 홈은 노션 워크스페이스 형태의 좌측 사이드바(로고·검색·루트/루트 폴더 단순 메뉴·로그아웃) + 우측 작업 영역(상단 브레드크럼, 페이지 타이틀, 파일 테이블, 업로드)으로 구성된다. 사이드바는 향후 기능 확장을 고려해 단순한 메뉴 구조를 유지한다 — 폴더 트리 등 복잡한 내비게이션은 아직 없다. 모바일(`sm` 미만)에서는 사이드바가 숨고 상단 헤더에 브레드크럼과 로그아웃 아이콘만 남는다.
